...
import { getTransicao100 } from "../api";
...
export const Transicao100 = () => {
    const [loading, setLoading] = useState(true);
    const { customerId } = useParams();
    const navigate = useNavigate();

    const load = async () => {
        let transicao100 = await getTransicao100(customerId, navigate);
            ...
            <Accordion allowToggle defaultIndex={0} alignSelf={"flex-start"} w="100%">
                    ...
                    <AccordionPanel>
                        <VStack w="100%" p={3} border={"1px solid gray"} rounded="lg">
                            <Text textAlign={"center"}>
                                60 anos de idade para homens, no momento da requisição
                                <br />
                                57 anos de idade para mulheres, no momento da requisição
                                <br />
                ...
                <AccordionItem>
                    ...
                    <AccordionPanel display="flex" rowGap={2} flexDirection="column">
                        <Box>
                            <Text>
                                Essa Regra de Transição é válida tanto para os contribuintes do INSS (do Regime Geral da
                                Previdência Social), quanto para os servidores públicos (aqueles do Regime Próprio da
                                Previdência Social). Isso significa que, além da idade mínima e do tempo de
                                contribuição, o segurado ainda terá que cumprir o Pedágio de 100%, que é o tempo que
